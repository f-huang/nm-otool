...
/*   Updated: 2019/02/12 14:30:11 by fhuang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int64_t	got_my_host(struct fat_arch *fat, uint32_t nfat_arch, uint8_t swap)
{
	uint64_t			i;
...
static void	output(const char *filename, uint32_t nfat_arch, uint64_t cputype)
{
...
static void	parse_object(t_otool *otool, struct fat_header *header, uint32_t offset, const char *filename)
{
	void				*ptr;
...
void	otool_fat_32(t_otool *otool, void *ptr, const char *filename, uint8_t swap)
{
	...
	if ((i = got_my_host(fat, nfat_arch, swap)) != -1)
		parse_object(otool, header, swap_32(fat[i].offset, swap), filename);
	else
		while (++i < nfat_arch)
		{
			output(filename, nfat_arch, swap_32(fat[i].cputype, swap));
			parse_object(otool, header, swap_32(fat[i].offset, swap), NULL);
		}
}
