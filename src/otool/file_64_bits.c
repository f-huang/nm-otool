...
/*   Updated: 2019/02/13 16:10:01 by fhuang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_section(struct section_64 *section, const void *ptr, void *end_of_file, uint8_t swap)
{
	uint64_t	size;
	uint64_t	i;
	uint64_t	j;
	...
	if (!is_ptr_in_file(end_of_file, ptr + size) || !is_ptr_in_file(end_of_file, tmp + size))
		return ;
	while (i < size)
	{
...
static void	parse_sections_64(t_nm_otool *nm_otool, struct segment_command_64 *segment,\
	const void *ptr, uint8_t swap)
{
	...
	while (is_ptr_in_file(nm_otool->end_of_file, section + i) && i < swap_64(segment->nsects, swap))
	{
...
void		otool_64_bits(t_nm_otool *nm_otool, void *ptr, const char *filename, uint8_t swap)
{
	...
	while (is_ptr_in_file(nm_otool->end_of_file, lc) && i < swap_64(header->ncmds, swap))
	{
		if (swap_64(lc->cmd, swap) == LC_SEGMENT_64)
			parse_sections_64(nm_otool, (struct segment_command_64*)lc, ptr, swap);
